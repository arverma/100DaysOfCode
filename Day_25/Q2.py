...
def partition(a, l, r):
    ...
    index = random.randint(l,r)     # Random initilising
    a[index], a[r] = a[r], a[index] # Swap it to put that element at last 
    
    x = a[r]
    k = l-1
    for j in range(l, r):
         # For finding the greatest use '>'
         # For finding the smallest use '<'
        if(a[j] > x): 
            k += 1
            a[k], a[j] = a[j], a[k]
    k += 1
...
l = 0 
r = n-1
print('K\'th largest element is ',selection(a, key, l, r))